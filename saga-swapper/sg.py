...
class Clear:
    def clear(self):
        ...
        print(f"""{blue}
saga swapping tool @saga.fastest       
    {reset}\n""")
...
class Swapper:
    ...
    def disc(self):
        try:
            ...
            e.add_field(name=f"**User**: **{self.target}**")
            e.add_field(name=f"*Attempts*: **{self.attempts}**")
            e.set_thumbnail(url=self.webthumb)
            webhook.send(username="saga #666",avatar_url=self.webpfp,embed=e)
        except Exception as e:
            pass
    ...
    def editProfile(self,username):
        self.setProxy()
        data = {'_uuid': self.uuid,'_uid': self.username_id,'_csrftoken': self.token,'external_url': "",'phone_number': self.phone,'username': username,'full_name': self.first_name,'biography': self.bio,'email': self.email,'gender': 3}
    ...
    def stop(self):
        ...
        if check.status_code != 404 and self.claimed:
            ...
            self.disc()
            ctypes.windll.user32.MessageBoxW(0, f'Successfully Swapped : @{self.target} | Attempts: {self.attempts}', 'Saga Swapper', 0)
            input(f"\n{red}[+] Press ENTER to exit....")
            exit(0)
        print(f'\n[{red}!{reset}] Spam Blocked : @{self.target}')
        ctypes.windll.user32.MessageBoxW(0, f'Rate Limit Reached! : @{self.target}', 'Saga Swapper', 0)
        input(f"\n{red}[+] Press ENTER to exit....")
        exit(0)
...
if __name__ == "__main__":
    ...
    ctypes.windll.user32.MessageBoxW(0, f'Are You ready!??', 'Saga Insta Swapper', 0)
    Start(m)
